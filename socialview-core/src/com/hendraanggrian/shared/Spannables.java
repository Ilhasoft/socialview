package com.hendraanggrian.shared;
...
public final class Spannables {
     ...
     */
    @NonNull
    public static Collection<Object> putSpansAll(@NonNull Spannable spannable, @NonNull Pattern regex, @Flags int flags, @NonNull SpanGetter2... getters) {
        Matcher matcher = regex.matcher(spannable);
        Collection<Object> spans = new ArrayList<>();
        while (matcher.find()) {
            int start = matcher.start();
            int end = matcher.end();
            for (SpanGetter2 getter : getters) {
                Object span = getter.getSpan(spannable.toString().substring(start, end).replaceFirst("@", ""));
                spannable.setSpan(span, start, end, flags);
                spans.add(span);
            }
        }
     ...
     */
    @NonNull
    public static Collection<Object> putSpansAll(@NonNull Spannable spannable, @NonNull String regex, @Flags int flags, @NonNull SpanGetter2... getters) {
        return putSpansAll(spannable, Pattern.compile(regex), flags, getters);
    }
     ...
     */
    @NonNull
    public static Collection<Object> putSpansAll(@NonNull Spannable spannable, @NonNull Pattern regex, @NonNull SpanGetter2... getters) {
        return putSpansAll(spannable, regex, SPAN_EXCLUSIVE_EXCLUSIVE, getters);
    }
     ...
     */
    @NonNull
    public static Collection<Object> putSpansAll(@NonNull Spannable spannable, @NonNull String regex, @NonNull SpanGetter2... getters) {
        return putSpansAll(spannable, regex, SPAN_EXCLUSIVE_EXCLUSIVE, getters);
    }
    ...
    public interface SpanGetter2 {
        @NonNull
        Object getSpan(String value);
    }
}